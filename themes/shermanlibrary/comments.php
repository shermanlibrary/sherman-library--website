...
<section id="respond" class="respond-form">
	...
	<?php if ( !is_user_logged_in() ) : ?>
		<h3 id="comment-form-title"><?php comment_form_title( __('Leave a Comment', 'bonestheme'), __('Leave a Reply to %s', 'bonestheme' )); ?></h3>
	<?php endif; ?>
	...
	<?php if ( get_option('comment_registration') && !is_user_logged_in() ) : ?>
  	<div>
  		<p><?php printf( 'You must be %1$slogged in%2$s to post a comment.', '<a href="http://sherman.library.nova.edu/sites/login">', '</a>' ); ?></p>
  	</div>
	<?php else : ?>
	...
	<fieldset>
		<header>
			...
			<small class="comments-logged-in-as"><?php _e("Logged in as", "bonestheme"); ?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo wp_logout_url(get_permalink()); ?>" title="Log out of this account"><?php _e("Log out", "bonestheme"); ?> &raquo;</a></small>
		</header>
		...
		<ol>
			...
			<li>
				<button class="lib-button-small gradient--vertical" name="submit" type="submit" id="submit" class="button" tabindex="5" value="<?php _e('Submit', 'bonestheme'); ?>">Submit</button>
			  	<?php comment_id_fields(); ?>
		  	</li>
						
			<li>
				<p><strong>XHTML:</strong> <?php _e('You can use these tags', 'bonestheme'); ?>: <code><?php echo allowed_tags(); ?></code></p>
			</li>
